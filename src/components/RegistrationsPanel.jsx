...
import { Search, Check, X, Ban, Pencil, UserPlus } from "lucide-react";
...
export default function RegistrationsPanel({
  ...
  return (
    <div className="space-y-2">
      ...
      {filtered.map(r => {
        // Not a real <button> - the row already contains several (attendance ticks, the
        // attending-type toggle, the edit control), and a <button> can't nest another <button>
        // without the browser silently mis-parsing it. role="button" + keyboard handling keeps
        // it accessible without that restriction.
        const clickable = canManage && !!onUpdate;
        return (
          <div
            key={r.id}
            {...(clickable ? {
              onClick: () => setEditingReg(r), role: "button", tabIndex: 0,
              onKeyDown: e => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setEditingReg(r); } },
            } : {})}
          ...
          >
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                ...
                {canManage && onUpdate && (
                  <span onClick={e => { e.stopPropagation(); setEditingReg(r); }} className="whmi-btn-ghost !p-1.5" title="Edit registration"><Pencil size={13} /></span>
                )}
              </div>
            </div>
